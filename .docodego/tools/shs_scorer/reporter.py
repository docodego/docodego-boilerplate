"""Output formatters for SHS scoring results."""
...
def format_text(
    result: SHSResult, *, filename: str = "", threshold: int = 60,
) -> str:
    """Format SHS result as human-readable text report."""
    return _format_text(
...
def format_json(
    ...
    """Format SHS result as audit JSON."""
    return _format_json(
        result, tool_key=TOOL_KEY, filename=filename, threshold=threshold,
    )
